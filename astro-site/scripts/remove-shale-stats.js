...
for (const slug of shalePages) {
	...
	// Remove stats array declaration (multiline match)
	content = content.replace(/const stats = \[[\s\S]*?\];\s*\n/g, '');
	
	// Remove stats prop from ShalePlayPage component call
	content = content.replace(/,\s*stats=\{stats\}/g, '');
	content = content.replace(/stats=\{stats\},\s*/g, '');
